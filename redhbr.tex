...
Para o lancamento do Repositório Digital das Humanidades (PT-BR) - REDHBR, nós escolhemos o \href{https://dhcenternet.org/initiatives/day-of-dh/2021}{Dia Internacional das Humanidades Digitais} em 29/04/2021.
...
\textbf{Download}: \href{http://u.pc.cd/hhFotalK}{Se preferir baixar a base dos PDFs sem usar o código clique aqui}.
...
\textbf{Download}: \href{http://u.pc.cd/AcbrtalK}{Se preferir baixar a planilha sem usar o código clique aqui}.
...
\hypertarget{scielo.br}{%
\chapter{Scielo.br}\label{scielo.br}}

\hypertarget{o-que-uxe9-scielo}{%
\section{O que é Scielo?}\label{o-que-uxe9-scielo}}

Scielo (\emph{Scientific Electronic Library Online} e que ) é uma biblioteca digital que reune uma vasta coleção de periódicos científicos brasileiros. Segundo \href{https://www.scielo.br}{site oficial}, consiste seu objetivo é ser:

\begin{quote}
Biblioteca eletrônica que possa proporcionar um amplo acesso a coleções de periódicos como um todo, aos fascículos de cada título de periódico, assim como aos textos completos dos artigos. O acesso aos títulos dos periódicos e aos artigos pode ser feito através de índices e de formulários de busca.
(\ldots)
...
\tightlist
\item
  A primeira delas, \texttt{scielo\_scraper}, permite que o usuário define uma das oito áreas de conhecimento disponíveis na coleção do Scielo.br para efetuar uma raspagem completa (é possível optar se pretende raspar os arquivos PDF ou apenas os arquivos XML);
\item
  A segunda, \texttt{scielo\_ISSN}, permite que o usuário defina uma lista específica de revistas que pretende raspar através ISSN de cada uma delas. Assim como na ferramenta anterior, é possível definir o tipo de raspagem (PDF ou XML);
\item
  A terceira ferramente é específica para tratamento dos dados gerados pelas raspagens dos arquivos XML. A \texttt{scielo\_xml\_to\_csv} analisa, seleciona, organiza e salva as informações do dataset de arquivos XML das revistas previamente baixados a partir das ferramentas anteriores.
\end{itemize}

Abaixo, detalhamos cada uma delas.

\hypertarget{scripts-de-raspagem-1}{%
\section{Scripts de raspagem}\label{scripts-de-raspagem-1}}

\href{https://github.com/LABHDUFBA/ferramentas_scielo}{\textbf{Clique aqui para acessar o repositório no Github}}

\hypertarget{scielo_scraper---raspagem-por-uxe1rea-do-conhecimento}{%
\subsection{scielo\_scraper - Raspagem por área do conhecimento}\label{scielo_scraper---raspagem-por-uxe1rea-do-conhecimento}}

Esse script permite ao usuário selecionar qual assunto ele pretende raspar de acordo com a categorização estabelecida pela plataforma \href{https://www.scielo.br/scielo.php?script=sci_subject\&lng=pt\&nrm=iso}{Scielo.br}. Após rodar o \texttt{run\_scielo\_scraper.py} é possível escolher entre oito assunto:
...
:warning: Os arquivos XML possuem todos os metadados dos artigos, incluíndo as referências bibliográficas). Após o download dos arquivos XML é posssível utilizar a ferramenta \texttt{scielo\_xml\_to\_csv} para convertar todos os XML para uma planilha. Para mais informações leia o \href{https://github.com/LABHDUFBA/ferramentas_scielo/blob/master/scielo_xml_to_csv/README.md}{README.md} da ferramenta.

\includegraphics{img/scielo_ex1.png}

\hypertarget{scielo_issn---raspagem-por-issn}{%
\subsection{scielo\_ISSN - Raspagem por ISSN}\label{scielo_issn---raspagem-por-issn}}

Nesse script é possível raspar uma revista ou uma lista de revistas através do ISSN.

Possui as mesmas características do \texttt{run\_scielo\_scraper.py}, porém a definição da(s) revista(s) a ser raspada é feita ISSN.

\includegraphics{img/scielo_ex2.png}

\hypertarget{dados-4}{%
\section{Dados}\label{dados-4}}
...
\tightlist
\item
  \texttt{scielo/pdf/nomeDaRevista} no caso da raspagem de PDFs;
\item
  \texttt{scielo/xml/nomeDaRevista} no caso da raspagem de XMls.
\end{itemize}

:warning: Atenção: Se a pasta com o nome de uma revista já exixtir, o algoritmo entenderá que a raspagem dela já foi efetuada e \textbf{passará para a revista seguinte seguinte}.

\textbf{Download}: \href{http://u.pc.cd/ntkrtalK}{Se preferir baixar a base dos XML das revistas da área de \emph{Ciências Humanas} sem usar o código clique aqui}. Esse arquivo zipado é formado pelos arquivos XML de 91 revistas categorizada como Ciências Humanas pelo Scielo.br, totalizando \textbf{72.552 itens}. Essa raspagem foi realizada em 22 de novembro de 2020 e o arquivo .zip tem \textbf{890.2 MB.}

\hypertarget{scielo_xml_to_csv---tratamento-dos-dados}{%
\section{scielo\_xml\_to\_csv - Tratamento dos dados}\label{scielo_xml_to_csv---tratamento-dos-dados}}

\href{https://github.com/LABHDUFBA/ferramentas_scielo/tree/master/scielo_xml_to_csv}{\textbf{Clique aqui para acessar o repositório no Github}}

\textbf{Scripts para converter Dataset de XML das revistas do \emph{Scielo} (previamente baixado) para CSV.}

Esses scripts tem como objetivos analisar, selecionar, organizar e salvar informações de um dataset de arquivos XML de todas as revistas previamente baixados a partir das \texttt{ferramentas\_scielo}

O \texttt{run.py} , após solicitar que o usuário defina o caminho da pasta com o dataset, acessa a mesma e analisa cada XML, inserindo os dados em um arquivo CSV salvo com o nome \texttt{metadata\_\{revista\}.csv}, numa pasta \texttt{scielo/xml/CSVs}.

:warning: \emph{É preciso definir o caminho do diretório com o dataset. Essa pasta deve conter pastas de cada revista com seus arquivos XML a serem analisados.}

\includegraphics{img/scielo_ex3.png}
...
\item
  index,
\item
  file\_name: nome do arquivo,
\item
  article\_id: identificação do arquivo,
\item
  authors: lista de autores,
\item
  authors affiliation: lista de filiações,
\item
  article\_title: título do artigo,
\item
  journal\_title: título do revista,
\item
  journal\_issn: ISSN da revista,
\item
  journal\_publisher: instituição da revista,
\item
  pub\_date: data da publicação,
\item
  abstract: resumo,
\item
  key\_words: lista de palavras-chave,
\item
  volume: volume,
\item
  num: número,
...
Em seguida, com a função \texttt{df\_final()}, todos os arquivos CSV são unidos (ou concatenados) em um único dataframe com \texttt{Pandas} e salvos em um CSV chamado \texttt{metadata\_scielo\_\{yyyy-mm-dd\_H-M-S\}.csv} na pasta \texttt{scielo/xml/CSVs/metadados-scielo}. Nesse csv final há uma nova coluna chamada original\_index, onde consta a numeração do index original de cada revista e um novo index geral é criado (consta na atualização de 04/05/2021).

\textbf{Download}: \href{http://u.pc.cd/ba27}{Se preferir baixar a base dos CSV de \emph{Ciências Humanas} sem usar o código clique aqui}. Esse arquivo zipado é formado pelos arquivos CSV das 91 revistas categorizada como Ciências Humanas pelo Scielo.br previamente raspados usando a ferramenta \texttt{scielo\_scraper} em 22 de novembro de 2020. A execução do \texttt{scielo\_xml\_to\_csv}aconteceu em 04/04/2021 e gerou esse arquivo zipado com todos os 91 CSVs de cada revista mais o CSV final, com todos os \textbf{72.552 itens} e possui \textbf{234.4 MB}.

Elementos presentes nesse repositório foram retirados de \href{https://github.com/johnsgomez/Scielo_Journal_Metadata_Downoader}{Scielo\_Journal\_Metadata\_Downoader}, criado por \href{https://github.com/johnsgomez}{johnsgomez}
